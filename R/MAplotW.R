...
MAplotW <- function(Mvalue, Avalue=NULL, filtFin=NULL, ProjNa=NULL, FCthrs=NULL, subTxt=NULL, grayIncrem=TRUE,
  useComp=1, compNa=NULL, batchFig=FALSE, cexMa=1.8, cexLa=0.7, limM=NULL, limA=NULL, cexPt=NULL, cexSub=NULL, useMar=c(6.2,4,4,2), callFrom="", silent=FALSE,debug=FALSE) {
  ## MA plot
  ...
  if(length(Mvalue) <1) message(" nothing to do, 'Mvalue' seems to be empty !") else  {
    ...
    if("MArrayLM" %in% class(Mvalue)) {
      useFdrTy <- "BH"              # initiate
      groupSep <- "-"               # initiate
      chMeans <- "means" %in% names(Mvalue)
      if(!chMeans) stop("no element 'means' found in 'Mvalue'")
      if(ncol(Mvalue[[which(chMeans)]]) >2) {    # will need to make choice of which $means to use
        useFdrTy <- c("FDR","BH","p.value","t")[which(c("FDR","BH","p.value","t") %in% names(Mvalue))]
        if(length(useFdrTy) <1) stop("no element like 'p.value' found for matching pair-wise names to 'useComp'") else useFdrTy <- useFdrTy[1]
        } else useFdrTy <- NULL
      ...
      avCol <- .sampNoDeMArrayLM(Mvalue, useComp, lstP=useFdrTy,silent=silent,callFrom=fxNa)
      if(length(Mcol) >0) Mcol <- Mcol[1] else {     
        Mvalue$Mval <- Mvalue$means[,avCol[1]] - Mvalue$means[,avCol[2]]          
        Mcol <- which(names(Mvalue)=="Mval")
        subTxt <- c(subTxt, wrMisc::pasteC(colnames(Mvalue$means)[1:2],quoteC="'"))
        if(!silent & ncol(Mvalue$means) >2) message(fxNa," Using ", wrMisc::pasteC(colnames(Mvalue$means)[avCol],quoteC="'"), " for calulating M-values")  
      }    
      ## look for A values (if external, see if usable)
      ...
      if(length(Avalue) <1) {
        ## no explicit Avalue, try to extract from MArrayLM-object (Mvalue$means)
        Avalue <- (Mvalue$means[,avCol[1]] + Mvalue$means[,avCol[2]])/2
      } 
      ## recuperate filtering - if present, but only when no custom filtering provided
      if(length(filtFin) <1 | identical(filtFin,TRUE)) {
    ...
    if(length(filtFin) >0) {
      ...
      } else if(length(filtFin)==nrow(merg)) merg <- cbind(merg, filtFin=filtFin)
    ...
    if(sum(filtFin,na.rm=TRUE) <1) {               # & !grayIncrem
      if(length(FCthrs) <1) {       # no FCthrs, grey incement only on filtFin
        graphics::points(Avalue[filtFin], Mvalue[filtFin], pch=16, cex=useCex, col=grDevices::rgb(0.4,0.4,0.4,alph)) }
    } else  {
      passFC <- which(filtFin & abs(Mvalue) > FCthrs)
      alph2 <- max(round(1.3/(10+sum(filtFin)^0.25),2), alph+0.05)
      if(!silent) message(" n=",length(Mvalue),"  FCthrs=",signif(2^FCthrs,2),"  filt=",sum(filtFin),"  passFC=",length(passFC), 
        if(debug) c("  range Mva ",wrMisc::pasteC(signif(range(Mvalue,na.rm=T),3)),"  alph=",alph,"  useCex=",useCex,"  alph2=",alph2))
      if(length(passFC) >0) {
