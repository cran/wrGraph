...
This object contains data for different types of plots (MA-plot, Volcano-Plot, etc ..), let's also mark the names of those passing the fold-change threshold.

```{r MA4b,fig.height=6, fig.width=8, fig.align="center",  echo=TRUE}
...
Please note that the examples use faily small toy-data where the estimation of local fdr-values (lfdr) may not be optimal. 
For this reason the functions for estimating the lfdr give warning messages which are suppressed in the exmaples below.
In real world testing the user may either switch to the frequenly used Bejamini-Hochberg (global) FDR or should interprete results on lfdr-level with caution.
...
Unfortunately the resulting html couldn't get integrated in this package vignette. You need to open in (separetely) using a browser or launch a browser out of R (see commented last line of code).
...
[CNRS](http://www.cnrs.fr/en), [Universit&eacute; de Strasbourg (UdS)](https://www.unistra.fr/fr) and [Inserm](https://www.inserm.fr).
...
The author wishes to thank the [CRAN-staff](https://CRAN.R-project.org) for all their help and their efforts in maintaining this repository of R-packages.
...
For completeness, here detailed documentation of versions used to produce this document, produced by the function sessionInfo().
