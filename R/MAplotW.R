...
#' @param filtFin (logical) optional filter (matrix of \code{TRUE}/\code{FALSE}) to apply to data before plotting 
...
MAplotW <- function(Mvalue,Avalue=NULL,filtFin=NULL,ProjNa=NULL,FCthrs=NULL,subTxt=NULL,grayIncrem=TRUE,
  ...
  defFiltFin <- FALSE             # don't allow processing using custom filtering
  if(debug) silent <- FALSE
  if(length(Mvalue) <1) stop(" nothing to do, 'Mvalue' seems to be empty !")
  if("MArrayLM" %in% class(Mvalue)) {
    ...
    ## recuperate filtering if present
    if(length(filtFin) <1) {
    ...
    ## this is case of explicit arguments
    if(length(Avalue) <1) stop(" argument 'Avalue' required (if 'Mvalue' not 'MArrayLM'-type object)")
  }
  ...
  ## apply filtering
  if(!defFiltFin) {   # not default fltering, allow filtering provided
